...
object Implementation extends Template {
  ...
  def interps(e:Expr, env:Env, ha:Value, k:Cont): Value = e match {
      ...
      case h :: t => interps(h, env, ha, hv => 
        t match{
          case head1 :: next1 => interps(head1, env, ha, head1v =>
            interps(TupleE(next1), env, ha, nextv => nextv match{
              case TupleV(values) => k(TupleV(hv :: head1v :: values))
              case _ => error()
            }))
          case Nil => k(TupleV(List(hv)))
        })
      case Nil => k(TupleV(List()))
    }
    ...
    case App(function, arguments) => interps(function, env, ha, fv =>
      interps(TupleE(arguments), env, ha, av => av match{
        ...
        case ConsV(h, t) => h match{
          case CloV(parameters, b, fenv) => t match{
            case ConsV(head, htail) => head match{
              case ContV(conti) => 
                interps(b, fenv, htail, expval => 
                  expval match{
                    case CloV(param, body, f2env) => param match{
                      case paramhead :: paramtail => if (paramtail == Nil) interps(body, f2env+(paramhead -> ev), htail, conti) else error()
                      case Nil => error()
                    }
                    case ContV(continuation) => continuation(ev)
                    case _ => error()
                  })
              case _ => error()
            }
