...
### java.lang.StringIndexOutOfBoundsException: offset 1516, count -7, length 8535
...
object Implementation extends Template {
  ...
  def exprtovallist(l: List[Expr], e: Env, h:Value, k:Cont): Value = {
    def exprtoval(list: List[Expr], env: Env, resultlist: List[Value], hand:Value, kt:Cont): Value = {
      list match{
        case head :: next => 
          interps(head, env, hand, hv => next match{
            case h :: t => interps(h, env, hand, h2v => k(TupleV(hv :: h2@@)))
            case Nil => k(TupleV(hv))
          }
            
            interps(TupleE(next), env, hand, ev => ev match{
            case TupleV(evalues) => k(TupleV(hv :: evalues))
            case _ => error()
          }))
