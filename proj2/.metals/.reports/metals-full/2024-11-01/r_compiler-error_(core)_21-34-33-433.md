...
object Implementation extends Template {
  ...
  def interps(e:Expr, env:Env, ha:Value, k:Cont): Value = e match {
    ...
    case App(function, arguments) => interps(function, env, ha, fv =>
      fv match{
        case CloV(params, body, fenv) => interps(TupleE(arguments), env, ha, av => 
          av match{
            case TupleV(values) => interparatoarg(params, values, body, fenv, ha, k)
            case _ => error()
          }
        )
        ...
        case _ => interps(TupleE(arguments), env, ha, av => k(error()))
      }
      )
